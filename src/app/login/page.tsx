      ...
      <div className="w-full max-w-[480px] space-y-12 flex flex-col items-center animate-in fade-in zoom-in duration-700">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.4em] opacity-80">Elite Badminton Analytics</p>
          </div>
        </div>
        ...
        <Card className="w-full shadow-[0_40px_100px_-20px_rgba(0,0,0,0.15)] border-none overflow-hidden bg-white rounded-[2.5rem]">
          <Tabs defaultValue="login" className="w-full" onValueChange={(v) => {
            ...
            <TabsList className="grid w-full grid-cols-2 rounded-none h-16 bg-muted/5 p-0 border-b border-muted/20">
              <TabsTrigger value="login" className="h-full rounded-none data-[state=active]:bg-white data-[state=active]:shadow-none font-black text-[10px] uppercase tracking-widest transition-all">SIGN IN</TabsTrigger>
              <TabsTrigger value="signup" className="h-full rounded-none data-[state=active]:bg-white data-[state=active]:shadow-none font-black text-[10px] uppercase tracking-widest transition-all">SIGN UP</TabsTrigger>
            </TabsList>
            ...
            <CardContent className="pt-10 px-10 pb-12 space-y-8">
              ...
              <Button 
                ...
                variant="outline"
                className="w-full h-14 text-xs font-black uppercase tracking-widest gap-4 border-2 border-muted/20 hover:bg-muted/30 transition-all shadow-xl shadow-black/5 rounded-2xl group active:scale-[0.98]" 
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[8px] uppercase">
                  <span className="bg-white px-6 text-muted-foreground font-black tracking-[0.3em]">SECURE ACCESS</span>
                </div>
              </div>

              <form onSubmit={handleEmailAuth} className="space-y-5">
                <div className="space-y-1.5">
                  <Label htmlFor="email" className="font-black text-[9px] uppercase tracking-[0.2em] text-muted-foreground px-1">Email Identity</Label>
                  <div className="relative">
                    ...
                    <Input 
                      ...
                      className="pl-12 h-14 bg-muted/5 border-2 border-muted/10 focus-visible:ring-primary shadow-inner rounded-2xl text-sm font-bold"
                      value={email}
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="password" className="font-black text-[9px] uppercase tracking-[0.2em] text-muted-foreground px-1">Access Key</Label>
                  <div className="relative">
                ...
                <Button type="submit" className="w-full h-14 font-black text-xs uppercase tracking-widest shadow-2xl shadow-primary/30 rounded-2xl transition-all hover:translate-y-[-2px] active:translate-y-[1px] group" disabled={isLoggingIn}>
                  {isLoggingIn ? (
                    <Loader2 className="mr-2 h-5 w-5 animate-spin" />
                  ) : (
                    <>
                      {isSignUp ? "Register Account" : "Enter Dashboard"}
                      <Trophy className="ml-3 h-4 w-4 transition-transform group-hover:rotate-12" />
                    </>
                  )}
                </Button>
              </form>

...
export default function Dashboard() {
  ...
  const chartConfig = {
    wins: { label: UI_STRINGS.dashboard.victories, color: "hsl(var(--secondary))" },
    losses: { label: UI_STRINGS.dashboard.defeats, color: "hsl(var(--destructive))" },
  };
        ...
        <header className="flex h-16 md:h-20 shrink-0 items-center justify-between gap-4 border-b bg-white/95 backdrop-blur-xl px-4 md:px-8 sticky top-0 z-50 shadow-sm w-full">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-lg md:text-xl font-black tracking-tight text-foreground uppercase">{UI_STRINGS.dashboard.tacticalIntel}</h1>
              <p className="text-[10px] text-muted-foreground uppercase font-bold hidden sm:block">
                {UI_STRINGS.dashboard.loggedAs} {user.displayName || 'Player'}
              </p>
            </div>
          </div>
          <Button asChild size="sm" className="md:size-default shadow-lg shadow-primary/20 rounded-full px-4 md:px-6">
            <Link href="/matches/new">
              ...
              <span>{UI_STRINGS.common.recordAction}</span>
            </Link>
          </Button>
        </header>
          ...
          <div className="relative w-full h-[200px] md:h-[350px] rounded-2xl md:rounded-[2rem] overflow-hidden shadow-xl bg-primary">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-primary/80 to-transparent flex flex-col justify-end p-6 md:p-12">
              <Badge className="w-fit mb-3 bg-white/20 text-white border-white/30" variant="outline">
                {matches.length === 0 ? UI_STRINGS.dashboard.newRoster : UI_STRINGS.dashboard.eliteCompetitor}
              </Badge>
              <h2 className="text-2xl md:text-5xl font-black tracking-tighter text-white mb-2 leading-none">
                {matches.length === 0 ? UI_STRINGS.dashboard.dominateCourt : UI_STRINGS.dashboard.smashLimits}
              </h2>
              <p className="max-w-xl text-white/90 text-sm md:text-lg leading-relaxed font-medium opacity-90">
                {matches.length === 0 
                  ? UI_STRINGS.dashboard.newRosterDesc
                  : UI_STRINGS.dashboard.eliteDesc.replace('{count}', matches.length.toString()).replace('{ratio}', stats.winRatio.toString())}
              </p>
            </div>
          </div>
          ...
          {matches.length > 0 ? (
            <>
              <div className="grid gap-4 md:gap-6 grid-cols-2 lg:grid-cols-4">
                {[
                  { icon: Zap, title: UI_STRINGS.dashboard.bestAlly, value: stats.bestAlly, color: "text-secondary" },
                  { icon: UserPlus, title: UI_STRINGS.dashboard.frequent, value: stats.frequentPartner, color: "text-primary" },
                  { icon: Swords, title: UI_STRINGS.dashboard.nemesis, value: stats.nemesis, color: "text-destructive" },
                  { icon: Heart, title: UI_STRINGS.dashboard.favRival, value: stats.favoriteRival, color: "text-secondary" }
                ].map((stat, i) => (
              ...
              <div className="grid gap-6 md:gap-8 grid-cols-1 lg:grid-cols-5">
                <Card className="lg:col-span-2 border-none shadow-sm bg-white rounded-xl md:rounded-2xl">
                  <CardHeader className="border-b bg-muted/5 p-4 md:p-6">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-sm md:text-lg font-black uppercase tracking-tight">{UI_STRINGS.dashboard.timelineAction}</CardTitle>
                      <Button variant="ghost" size="sm" asChild className="text-xs font-bold text-primary hover:bg-primary/5">
                        <Link href="/history">{UI_STRINGS.dashboard.viewAll}</Link>
                      </Button>
                    </div>
                  </CardHeader>
                ...
                <Card className="lg:col-span-3 border-none shadow-sm bg-white rounded-xl md:rounded-2xl">
                  <CardHeader className="border-b bg-muted/5 p-4 md:p-6">
                    <CardTitle className="text-sm md:text-lg font-black uppercase tracking-tight">{UI_STRINGS.dashboard.victoryMetrics}</CardTitle>
                  </CardHeader>
              ...
              <div className="grid gap-4 md:gap-6 grid-cols-2 lg:grid-cols-4 pb-12">
                {[
                  { icon: Activity, title: UI_STRINGS.dashboard.sessions, value: stats.totalMatches, color: "primary" },
                  { icon: Target, title: UI_STRINGS.dashboard.efficiency, value: `${stats.winRatio}%`, color: "secondary" },
                  { icon: Trophy, title: UI_STRINGS.dashboard.victories, value: stats.wins, color: "secondary" },
                  { icon: Frown, title: UI_STRINGS.dashboard.defeats, value: stats.losses, color: "destructive" }
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              {[
                { icon: Target, title: UI_STRINGS.dashboard.featurePrecision, desc: UI_STRINGS.dashboard.featurePrecisionDesc },
                { icon: Trophy, title: UI_STRINGS.dashboard.featureGrouping, desc: UI_STRINGS.dashboard.featureGroupingDesc },
                { icon: TrendingUp, title: UI_STRINGS.dashboard.featureAnalytics, desc: UI_STRINGS.dashboard.featureAnalyticsDesc }
              ].map((feature, i) => (

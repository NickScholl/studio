    ...
    <div className="flex min-h-screen bg-[#f8f9fc]">
      ...
      <SidebarInset className="flex flex-col">
        <header className="flex h-16 shrink-0 items-center justify-between gap-2 border-b bg-white/80 backdrop-blur-md px-6 sticky top-0 z-20 shadow-sm w-full">
          ...
          <Button asChild className="gap-2 shadow-xl shadow-primary/20 bg-primary hover:bg-primary/90 transition-all rounded-full px-6 h-10 font-black uppercase text-xs tracking-widest">
        ...
        <main className="flex-1 space-y-10 p-6 lg:p-10 max-w-full mx-auto w-full">
          <div className="relative w-full h-[360px] md:h-[400px] rounded-[2.5rem] overflow-hidden shadow-2xl bg-primary animate-in fade-in duration-1000">
            ...
            <div className="absolute inset-0 bg-gradient-to-tr from-primary via-primary/60 to-transparent flex flex-col justify-center px-10 md:px-16">
              <Badge className="w-fit mb-6 bg-white/10 hover:bg-white/20 text-white backdrop-blur-xl border-white/20 px-4 py-1.5 font-black uppercase tracking-[0.2em] text-[10px]" variant="outline">
          ...
          {matches.length > 0 ? (
            <>
                ...
                {[
                  ...
                  <Card key={i} className="border-none shadow-xl shadow-black/5 hover:translate-y-[-4px] transition-all duration-300 rounded-[2rem] bg-white overflow-hidden group">
                    ...
                    <CardContent className="px-6 pb-6">
                      ...
                      <p className="text-[9px] mt-1 font-black uppercase tracking-widest text-muted-foreground/50">{stat.subtitle}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
                  ...
                  <CardHeader className="border-b bg-muted/5 p-8">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <CardTitle className="text-xl font-black tracking-tight uppercase">Recent Action</CardTitle>
                          <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-60">Last 5 Matches</CardDescription>
                        </div>
                      </div>
                      <Button variant="ghost" size="sm" asChild className="h-8 px-4 text-primary hover:bg-primary/5 font-black uppercase text-[9px] tracking-widest rounded-full">
                  ...
                  <CardContent className="flex-1 p-0">
                    <div className="divide-y divide-muted/30">
                      {recentMatches.map((match) => (
                        <div key={match.id} className="flex items-center justify-between p-6 hover:bg-muted/10 transition-all group">
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <p className="text-sm mt-1.5 font-mono font-black text-foreground/70 tracking-tighter">
                ...
                <Card className="lg:col-span-3 border-none shadow-2xl shadow-black/5 flex flex-col overflow-hidden bg-white rounded-[2.5rem]">
                  <CardHeader className="border-b bg-muted/5 p-8">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <CardTitle className="text-xl font-black tracking-tight uppercase">Win Distribution</CardTitle>
              ...
              <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <Card key={i} className={`border-none shadow-xl shadow-black/5 hover:translate-y-[-4px] transition-all duration-300 rounded-[2rem] overflow-hidden ${stat.invert ? 'bg-primary text-primary-foreground' : 'bg-white'}`}>
                    ...
                    <CardContent className="px-6 pb-6">
                      ...
                      <p className={`text-[9px] mt-1 font-black uppercase tracking-widest ${stat.invert ? 'opacity-50' : 'text-muted-foreground/50'}`}>{stat.subtitle}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
              {[
                ...
                <Card key={i} className="border-none shadow-2xl shadow-black/5 p-8 bg-white rounded-[2.5rem]">
                  <CardContent className="p-0">
                    <div className="h-16 w-16 rounded-[1.5rem] bg-primary/10 flex items-center justify-center mb-8">
                      <feature.icon className="h-8 w-8 text-primary" />
                    </div>
                    <h3 className="font-black text-2xl mb-3 tracking-tighter uppercase">{feature.title}</h3>
